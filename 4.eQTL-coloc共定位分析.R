# eQTLs_eQTLGen_finngen_R10_O15_PREECLAMPS----
library(data.table)
library(tidyverse)
...
gwas2 <- fread('../data/QTL_Coloc_Prepare/eQTLs_coloc.txt')
## 添加一下列名
...
for (i in unique(dat1$probeID)) { # 循环对每个筛选到的结果进行共定位，eQTL对每个基因基因共定位分析，mQTL对每个甲基化位点进行共定位分析，pQTL对每个蛋白进行共定位分析。
  ...
  neqtl = 31684 #eqtl样本量31684、mQTL样本量1980 、pqtl样本量有3个数据来源（参考以下），具体需根据实际分析使用数据来确定。
  ...
  if(result$summary[6] > 0.5){
    ...
    write.table(need_result,
                paste0("../result/coloc_result/coloc-cis-eQTLs_eQTLGen_finngen_R10_O15_PREECLAMPS-",SNP,".tsv"),
    ...
    write.table(gwas,
                paste0("../data/locusdata/gwas-cis-eQTLs_eQTLGen_finngen_R10_O15_PREECLAMPS-",i,".tsv"),
                col.names = T,row.names = F,sep="\t",quote = F)
    write.table(eqtl,
                paste0("../data/locusdata/qtl-cis-eQTLs_eQTLGen_finngen_R10_O15_PREECLAMPS-",i,".tsv"),
                col.names = T,row.names = F,sep="\t",quote = F)
    library(locuscomparer)
    locuscompare(paste0("../data/locusdata/gwas-cis-eQTLs_eQTLGen_finngen_R10_O15_PREECLAMPS-",i,".tsv"),
                 paste0("../data/locusdata/qtl-cis-eQTLs_eQTLGen_finngen_R10_O15_PREECLAMPS-",i,".tsv"),
                 title1 = 'GWAS',title2 = 'eQTL',#title1参数表示可视化结果的横坐标标签，默认是eQTL，因输入数据1为GWAS数据，因此title1 = GWAS，eQTL和pQTL的共定位分析，记得修改title2参数，如title2 = “eQTL"
                 legend =T)  
    ggsave(paste0("../figure/locus-cis-eQTLs_eQTLGen_finngen_R10_O15_PREECLAMPS-",i,".jpeg"),
           width=10,height = 6)
  }
...
write.table(output_table,file = '../result/coloc_result/coloc-merge-eQTLs_eQTLGen_finngen_R10_O15_PREECLAMPS.txt',
            row.names = F,quote = F)


ls ../figure/locus-cis-eQTLs*|wc -l